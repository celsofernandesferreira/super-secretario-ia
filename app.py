...
def inicializar_bd():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS historico_global (
            id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT, session_id TEXT, role TEXT, content TEXT
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS high_scores (
            id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT, nome TEXT, pontor INTEGER
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cache_horarios (
            linha TEXT PRIMARY KEY, url TEXT, conteudo_txt TEXT, ultima_atualizacao TEXT
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cache_titulos (
            tipologia TEXT PRIMARY KEY, descricao TEXT, preco TEXT, custo_cartao TEXT, prazo TEXT,
            documentos_json TEXT, ultima_atualizacao TEXT
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cache_tarifario (
            id INTEGER PRIMARY KEY CHECK (id = 1), url_pdf TEXT, conteudo_txt TEXT, ultima_atualizacao TEXT
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cache_paragens_linha (
            linha TEXT, paragem TEXT, PRIMARY KEY (linha, paragem)
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cache_titulo_linha (
            linha TEXT PRIMARY KEY, titulo TEXT, ultima_atualizacao TEXT
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cache_paragem_freguesia (
            paragem TEXT PRIMARY KEY, freguesia TEXT, fonte TEXT, ultima_atualizacao TEXT
        )
    """)
    conn.commit()
    conn.close()
...
def guardar_mensagem_bd(session_id, role, content):
    try:
        ...
        cursor.execute("INSERT INTO historico_global (timestamp, session_id, role, content) VALUES (?, ?, ?, ?)", (timestamp, session_id, role, content))
        conn.commit()
        conn.close()
...
def guardar_score_bd(nome, pontor):
    try:
        ...
        cursor.execute("INSERT INTO high_scores (timestamp, nome, pontor) VALUES (?, ?, ?)", (timestamp, nome, pontor))
        conn.commit()
        conn.close()
    except Exception as e:
...
def obter_avisos_facebook():
    ...
    try:
        ...
        2. Se a data limite já passou, considera o aviso EXPIROU.
...
def renderizar_rodape_anuncios(anuncios_ativos):
    ...
    <div class="footer-wrapper">
        <div class="disclaimer">⚠️ Aviso importante: Esta é uma ferramenta de apoio e verificação preliminar. Não é um canal oficial de submissão à Guimabus.</div>
    ...
    <script>
        ...
        async function correrAviso() {{
            ...
            if (a.imagem && a.imagem.trim() !== "") {{ img.src = a.imagem; img.style.display = "block"; img.style.visibility = "visible"; }}
            ...
            function animar() {{
                pos -= 2;
                txt.style.left = pos + "px";
                if (pos < -txt.offsetWidth) {{ indice = (indice + 1) % anuncios.length; setTimeout(correrAviso, 2000); }}
...
def gerar_mapa_linha_html(linha_id):
    """Gera o mapa visual folium. NÃO é uma ferramenta de chat (function calling) — devolve
    um caminho de ficheiro, que o texto do chat não consegue mostrar. É chamada diretamente
    pela secção "🗺️ Ver Mapa de Linha" da sidebar, que lê o ficheiro e desenha-o no ecrã."""
    ...
    if not paragens_da_linha:
        return None, "Sem paragens em cache para esta linha. Sincroniza os horários primeiro."
    coordenadas_rota = []
    paragens_com_coord = []
    for paragem in paragens_da_linha:
        ...
        for k, v in MAPA_LOCAL.items():
            if chave_p in k or k in chave_p:
                ...
                paragens_com_coord.append({"nome": paragem, "lat": v["lat"], "lon": v["lon"]})
                break
    if not paragens_com_coord:
        return None, "Sem dados geográficos suficientes no JSON para mapear esta linha (nenhuma paragem foi encontrada no geo_guimaraes.json)."
    ...
    for p in paragens_com_coord:
        ...
        folium.Marker(location=[p["lat"], p["lon"]], popup=folium.Popup(popup_text, max_width=300), icon=folium.Icon(color="green", icon="bus", prefix="fa")).add_to(mapa)
    ...
    return caminho_ficheiro, f"Mapa gerado com {len(paragens_com_coord)}/{len(paragens_da_linha)} paragens localizadas geograficamente."
...
def sincronizar_todos_horarios_guimabus():
    ...
    try:
        ...
        for linha_id, url_pdf in links_pdf.items():
            ...
            for tentativa in range(2):
                try:
                    ...
                    cursor.execute("INSERT OR REPLACE INTO cache_horarios (linha, url, conteudo_txt, ultima_atualizacao) VALUES (?, ?, ?, ?)", (linha_id, url_pdf, conteudo_final, timestamp_atual))
                    if linha_id in titulos_linha:
                        cursor.execute("INSERT OR REPLACE INTO cache_titulo_linha (linha, titulo, ultima_atualizacao) VALUES (?, ?, ?)", (linha_id, titulos_linha[linha_id], timestamp_atual))
        ...
        if linhas_falhadas: msg_sucesso += f" Falharam: {', '.join(linhas_falhadas)}."
...
def sincronizar_automaticamente_se_necessario(limite_dias: int = 7):
    if st.session_state.get("sync_automatico_tentado_nesta_sessao"): return
...
TIPOLOGIAS_PASSE_FALLBACK = {
    "Mensal": {
        ...
        "preco": "Consultar tabela tarifária", "custo_cartao": "5€",
...
def sincronizar_titulos_guimabus():
    ...
    try:
        ...
        for bloco in blocos:
            ...
            if match_preco: preco = match_preco.group(1).strip()
            elif re.search(r'\bGRATUITO\b', resto_texto, re.IGNORECASE): preco = "Gratuito"
            else:
            ...
            if match_docs:
                ...
                for d in candidatos_doc:
                    if re.match(r'^(Custo do cartão|Preço)', d, re.IGNORECASE): continue
                    documentos.append(d)
            if not documentos: documentos = ["Cartão de Cidadão / Documento de Identificação"]
            cursor.execute("INSERT OR REPLACE INTO cache_titulos (tipologia, descricao, preco, custo_cartao, prazo, documentos_json, ultima_atualizacao) VALUES (?, ?, ?, ?, ?, ?, ?)", (nome_tipologia, descricao, preco, custo_cartao, prazo, json.dumps(documentos, ensure_ascii=False), timestamp_atual))
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(pdf_response.content)) as pdf:
            for idx, pagina in enumerate(pdf.pages):
                ...
                if texto_pag: texto_extraido.append(f"[PÁGINA {idx+1}]\n{texto_pag}")
        conteudo_final = "\n\n".join(texto_extraido)
        if not conteudo_final.strip(): conteudo_final = "PDF em formato de imagem."
        conn = sqlite3.connect("agente_memoria.db")
        cursor = conn.cursor()
        timestamp_atual = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute("INSERT OR REPLACE INTO cache_tarifario (id, url_pdf, conteudo_txt, ultima_atualizacao) VALUES (1, ?, ?, ?)", (url_pdf, conteudo_final, timestamp_atual))
        conn.commit()
        conn.close()
...
def _extrair_paragens_de_texto(texto: str):
    ...
    for linha_texto in texto.split("\n"):
        ...
        if m:
            ...
            if len(nome) >= 3: paragens.add(nome)
    return paragens
    ...
    try:
        ...
        for linha_id, conteudo_txt in linhas_cache:
            ...
            for paragem in paragens:
                cursor.execute("INSERT OR IGNORE INTO cache_paragens_linha (linha, paragem) VALUES (?, ?)", (linha_id, paragem))
                total_paragens += 1
        conn.commit()
        conn.close()
...
def obter_freguesia_de_paragem(nome_paragem: str):
    ...
    for paragem, freguesia in todas:
        ...
        if match_direto or match_inverso: return freguesia
    return None
...
def planear_viagem_com_transbordo(origem: str, destino: str):
    ...
    if not todas:
        return "O índice de paragens ainda não foi construído. Peça ao administrador para sincronizar os horários."
    linhas_origem = set()
    ...
    if not linhas_origem:
        ...
        if paragens_da_freguesia:
            ...
            if linhas_origem: aviso_origem_por_freguesia = paragens_da_freguesia
    if not linhas_destino:
        ...
        if paragens_da_freguesia:
            ...
            if linhas_destino: aviso_destino_por_freguesia = paragens_da_freguesia
    if not linhas_origem:
...
def consultar_freguesia_paragem_tool(nome: str):
    if not nome: return "É necessário indicar o nome da paragem ou da freguesia."
    ...
    if paragens: return f"Paragens conhecidas na freguesia de '{nome}': {', '.join(paragens)}."
...
def obter_tipologias_cache():
    try:
        ...
        for tipologia, descricao, preco, custo_cartao, prazo, documentos_json in linhas:
            ...
            resultado[tipologia] = {"descricao": descricao, "preco": preco, "custo_cartao": custo_cartao, "prazo": prazo, "documentos": documentos}
        conn = sqlite3.connect("agente_memoria.db")
        cursor = conn.cursor()
        cursor.execute("SELECT MAX(ultima_atualizacao) FROM cache_titulos")
...
def consultar_tarifario_cache():
    try:
        ...
        resultado = cursor.fetchone()
        conn.close()
        if resultado: return f"Tabela tarifária (atualizada em {resultado[1]}):\n\n{resultado[0]}"
...
def consultar_tipologias_cache_tool():
    ...
    if not tipologias: return "Não existem tipologias de passe em cache."
...
def verificar_documentos_passe(tipologia: str, ficheiros_carregados: dict):
    ...
    if not nomes_documentos: return "Nenhum documento foi carregado ainda."
    try:
...
def recomendar_tipologias_passe(respostas: dict, tipologias_disponiveis: dict):
    ...
    def _tem(nome_parcial):
        for nome in tipologias_disponiveis:
            if nome_parcial.lower() in nome.lower(): return nome
        return None
...
def renderizar_jogo():
    ...
    <div style="text-align:center; background-color:#111; padding:15px; border-radius:10px; font-family:sans-serif;">
        ...
        <div style="margin-top: 15px; display: inline-block; width: 100%; text-align: center;">
            <div style="margin-bottom: 5px;"><button data-dir="cima" style="padding: 12px 24px; background: #34495e; color: white; border: none; border-radius: 5px; font-weight: bold; cursor: pointer; font-size: 18px;">▲</button></div>
        ...
        <script>
            ...
            function drawScene() {
                ...
                for(var i=tnt; i<canvas.height; i+=tnt) { ctx.beginPath(); ctx.moveTo(0, i); ctx.lineTo(gameWidth, i); ctx.stroke(); }
                ctx.fillStyle = '#2ecc71'; ctx.fillRect(gameWidth, 0, 3, canvas.height);
                ctx.fillStyle = '#3498db'; ctx.beginPath(); ctx.arc(apple.x + tnt/2, apple.y + tnt/2, (tnt-4)/2, 0, 2 * Math.PI); ctx.fill();
                ctx.fillStyle = '#ffffff'; ctx.beginPath(); ctx.arc(apple.x + tnt/2, apple.y + tnt/2, (tnt-12)/2, 0, 2 * Math.PI); ctx.fill();
                ...
                for(var k=0; k<10; k++) {
                    ...
                    if (leaderboard[k]) {
                        ...
                        ctx.textAlign = 'end'; ctx.fillText(item[1] + ' pas.', canvas.width - 15, yPos); ctx.textAlign = 'start';
                    } else { ctx.fillStyle = '#444'; ctx.fillText(medalha + '------', gameWidth + 15, yPos); }
                }
                if (gameOver) {
            ...
            function game() {
                if (gameOver) return;
                if (proximaDirecao) {
                    if (proximaDirecao.dx !== -dx || proximaDirecao.dy !== -dy) { dx = proximaDirecao.dx; dy = proximaDirecao.dy; }
                    proximaDirecao = null;
                }
                ...
                for (var i = 0; i < corpoParaVerificar.length; i++) {
                    if (corpoParaVerificar[i].x === head.x && corpoParaVerificar[i].y === head.y) { triggerGameOver(); return; }
                }
                snake.unshift(head);
                if (vaiComer) {
                    score += 10;
                    if (score % 50 === 0 && velocidadeMs > 80) { velocidadeMs -= 10; clearInterval(gameInterval); gameInterval = setInterval(game, velocidadeMs); }
                    apple = novaMaca();
                } else { snake.pop(); }
                drawScene();
            }
            function toggleGame() {
                ...
                if (!gameStarted) { gameStarted = true; btnAction.innerText = "Pause ⏸"; gameInterval = setInterval(game, velocidadeMs); }
                else { gameStarted = false; btnAction.innerText = "Play ▶"; clearInterval(gameInterval); }
            }
            ...
            function resetGame() { estadoInicial(); gameOver = false; gameStarted = true; btnAction.innerText = "Pause ⏸"; gameInterval = setInterval(game, velocidadeMs); drawScene(); }
...
MENSAGEM_INICIAL = """Olá! 👋 Sou o assistente virtual do Celso Ferreira.

Podes perguntar-me sobre:
- 🚌 **Guimabus** — horários, linhas, transbordos, tipologias de passe e tarifários
- 👨‍💻 **O Celso** — o seu percurso e porque procura oportunidades em IT
- 🎓 **Entrar em IT sem curso superior** — dicas e o que costuma ajudar
- 🛠️ **Este projeto** — como foi construído e as tecnologias usadas

Também simulo entrevistas técnicas (diz *"quero treinar para uma entrevista"*) e ajudo com problemas de helpdesk/suporte.
...
with st.sidebar:
    ...
    if "passe_ativo" not in st.session_state: st.session_state.passe_ativo = False
    ...
    st.subheader("🗺️ Mapa de Linha")
    if "mapa_ativo" not in st.session_state: st.session_state.mapa_ativo = False
    texto_botao_mapa = "Fechar Mapa X" if st.session_state.mapa_ativo else "Ver Mapa de Linha 🗺️"
    if st.button(texto_botao_mapa, use_container_width=True):
        st.session_state.mapa_ativo = not st.session_state.mapa_ativo
        st.rerun()
    st.divider()
...
📞 Contacto: **917 486 683**""")
    ...
    st.sidebar.caption(f"🗺️ Mapa estático: {len(MAPA_LOCAL)} locais carregados" if MAPA_LOCAL else "🗺️ geo_guimaraes.json não encontrado ou vazio.")
    st.sidebar.divider()
    ...
    if not st.session_state.admin_autenticado:
        ...
        if st.sidebar.button("🔍 Diagnosticar geo_guimaraes.json", use_container_width=True):
            if not MAPA_LOCAL:
                st.sidebar.error("Ficheiro não encontrado ou vazio. Confirma que 'geo_guimaraes.json' está na mesma pasta que app2.py.")
            else:
                amostra = list(MAPA_LOCAL.items())[:3]
                st.sidebar.success(f"{len(MAPA_LOCAL)} entradas carregadas.")
                for chave, dados in amostra:
                    st.sidebar.code(f"{chave}: {dados}", language="json")
        ...
        with st.sidebar.expander("🗄️ Histórico Permanente Global (BD)"):
            if os.path.exists("agente_memoria.db"):
                ...
                for r in reversed(linhas_bd):
                    ...
                    sessao = r[1]
                    if r[2] == "user": st.markdown(f"**🟢 [{hora_min}] Visitante ({sessao}):** {r[3]}")
                    else: st.markdown(f"**🤖 [{hora_min}] Agente ({sessao}):** {r[3]}")
                    st.divider()
...
if st.session_state.get("mapa_ativo"):
    st.subheader("🗺️ Ver Mapa de uma Linha")
    linha_para_mapa = st.text_input("Número da linha (ex: 170)", key="input_linha_mapa")
    if st.button("Gerar Mapa", key="btn_gerar_mapa"):
        if not linha_para_mapa.strip():
            st.warning("Indica o número da linha.")
        else:
            with st.spinner("A gerar o mapa..."):
                caminho, mensagem = gerar_mapa_linha_html(linha_para_mapa.strip())
            if caminho:
                st.success(mensagem)
    ...
    with st.chat_message("assistant", avatar="💼"):
        with st.spinner("Agente a processar contexto e ferramentas..."):
            try:
                ...
                PROMPT_EXECUTIVO = """Tu és o assistente virtual pessoal do Celso Ferreira.
                Respondes em Português de Portugal, de forma clara e direta, usando sempre a Knowledge Base e ferramentas quando relevante.

                O teu âmbito cobre quatro tipos de conversa:
                1. **Guimabus** (horários, linhas, transbordos, tipologias de passe, tarifário) — usa as ferramentas abaixo.
                2. **Sobre o Celso** — o seu percurso e motivação para procurar emprego em IT. Usa a Knowledge Base; se não souberes, di-lo com honestidade.
                3. **Conselhos sobre entrar em IT sem curso superior** — conselhos gerais e sensatos, deixando claro quando é opinião geral.
                4. **Sobre este projeto** — usa a Knowledge Base para explicar arquitetura, tecnologias e funcionalidades.

                Ferramentas relacionadas com a Guimabus:
                ...
                - consultar_cache_horario_linha: horários e tabelas fixas de uma linha. Devolve também o link oficial do PDF — inclui-o sempre na resposta.
                - consultar_tipologias_cache_tool: tipologias de passe.
                - consultar_tarifario_cache: tabela tarifária completa.
                - planear_viagem_com_transbordo: dado origem/destino (nomes de paragem), diz se há linha direta ou sugere transbordo.
                - consultar_freguesia_paragem_tool: diz em que freguesia fica uma paragem.
                - gerar_link_google_maps: dado o nome de um local (paragem, café, hospital, rua), devolve um link direto do Google Maps.
                - encontrar_paragem_mais_proxima: descobre a paragem oficial mais próxima de um café, fábrica ou ponto de interesse.

                LÓGICA DE PLANEAMENTO OBRIGATÓRIA: se o utilizador pedir direções para/de um local que NÃO É uma paragem (café, loja, fábrica), usa primeiro "encontrar_paragem_mais_proxima" para descobrir a paragem oficial mais próxima, e só depois "planear_viagem_com_transbordo" com essa paragem.

                Se alguém quiser VER um mapa visual de uma linha, diz que existe a secção "🗺️ Ver Mapa de Linha" na barra lateral — o mapa em si não pode ser mostrado dentro do texto do chat.

                Usa sempre consultar_tipologias_cache_tool e consultar_tarifario_cache para perguntas sobre preços/tipologias/documentos. Se quiser PEDIR um passe, aponta para "🎫 Pedir Passe" na barra lateral.

                REGRA ANTI-ALUCINAÇÃO — A MAIS IMPORTANTE DE TODAS:
                NUNCA inventes, estimes ou "preenchas" dados que as ferramentas ou a Knowledge Base não te deram. Usa SEMPRE e apenas a informação em "[DATA E HORA ATUAL DO SISTEMA]". Se não souberes, admite isso em vez de arriscar uma resposta errada."""
                ...
                # NOTA: gerar_mapa_linha_html NÃO está aqui — devolve um caminho de ficheiro que
                # o chat de texto não consegue mostrar. Essa função só é chamada diretamente pela
                # secção "🗺️ Ver Mapa de Linha" da sidebar (ver acima), que desenha o resultado.
                ferramentas_agente = [
                    ...
                    gerar_link_google_maps, encontrar_paragem_mais_proxima
                ]
